...
int check(struct gameState *before, struct gameState *after)
{
	...
	if(before->handCount[player] + 2 != after->handCount[player])
	{
		printf("incorrect hand count\n");
		test_flag = -1;
	}
	if(before->deckCount[player] != after->deckCount[player] + 3)
	{
...
int main () 
{
	...
	int i, n, num_players, seed, smithy_pos, test, deck, player;
	printf ("Random Test Smithy Start\n");
	...
	for (n = 0; n < num_tests; n++) 
	{
		...
		newgame->hand[player][smithy_pos] = smithy; 				//put adventurer in hand
		memcpy(&before, newgame, sizeof(struct gameState));					//save state before
		test = playCard(smithy_pos,0,0,0,newgame); 						//play adventurer
		if(test == -1)
		{
			test_counter++;
