...
bool is_positive_int_recurse(string numStr, int i) {
	//cout << "testing iteration " << i << " now " << endl;
	if(i < 0) {
		cout << "done" << endl;
		return true;
	}
	if(numStr.at(i) < '0' || numStr.at(i) > '9')		//cout << numStr.at(i) << endl;
		return false;
	else
...
int main () {
	...
	cin >> xStr >> yStr;
	while (true) {
		if(is_positive_int_recurse(xStr, (xStr.length()-1))){
			if(is_positive_int_recurse(yStr, (yStr.length()-1))){
				x = atoi(xStr.c_str());
				y = atoi(yStr.c_str());
				get(x, y);
				break;
			} else {
				cout << "One of your parameters did not work" << endl;
				main();
			}
		} else {
	 	  	cout << "One of your parameters did not work" << endl;
