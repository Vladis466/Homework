...
void too_Few(int tests,int quizzes,int assignments,int labs) {
	if (quizzes == 0 || tests == 0 || assignments == 0 || labs == 0) {
		cout <<  "Amount of inputs isn't satisfactory, meltdown initiated....Goodbye" << endl;
		exit(EXIT_FAILURE);
	}
}
...
/*********************************************************************
** Function: main
** Description: The main function where everything is called from. Takes values from the command line
** Parameters: Here we declare all variables that will be filled wth the weights 
** and number of inputs from each type. 
** Pre-Conditions: Initiation, values need to be entered in the command line as provided.
...
int main(int argc, const char *argv[]) {
	...
	int counter = 1;
	// While loop for the size of counter is convenient in this situation.
	while ( counter < argc)  {
		/*The way this works is it checks if each value is equal to the string form of the argv input,
		**The input will fit into one of the many categories provided. If the value isn't suitable it will kick you
		**out of the system. If it is suitable, the next value in the loop will be used as the number of tests etc..
		**Then it will call many other functions, testing the inputs for all type of gibberish.
		*/
	...
	too_Few(tests, quizzes, assignments, labs);
