...
{
	struct BSTree *newTree = malloc(sizeof(struct BSTree));
	assert(newTree != 0);
	initBSTree(newTree);
	return newTree;
}
...
int isEmptyBSTree(struct BSTree *tree) { 
	...
	return (tree->cnt == 0); /* temporary return val */
}
...
/*
 ...
 pre:	val is not null
 */
struct Node *_addNode(struct Node *cur, TYPE newVal)
{
	struct Node * node;
	if (cur == NULL)							//set equal to null?
	{	
	...
	else if (compare(cur->val, newVal) == -1)			//if newval is less than value at start
	{
		cur->left = _addNode(cur->left,newVal);
	}
	else
	{
		cur->right = _addNode(cur->right,newVal);
	}
...
{
	struct Node * Find = malloc(sizeof(struct Node)); 
	Find = tree->root;
	
	while (Find != NULL){
		if (compare(Find->val, val) == 0){
			return 1;
		}
		if (compare(tree->root->val, val) == -1){
			Find =Find->left, val;
		}
		if (compare(Find->val, val) == 1){
			Find = Find->right, val;
		}
	}
	return 0;
...
struct Node *_removeNode(struct Node *cur, TYPE val)
{
	...
	if (compare(cur->val, val) == 0)
	{
		...
		cur->val = _leftMost(cur->right);			//If right is not null, replace with leftmost of right.
		cur->right = _removeLeftMost(cur->right);
	
	}
	else if (compare(cur->val, val) == -1)			//if newval is less than value at start
	{
