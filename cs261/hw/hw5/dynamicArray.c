...
int _smallerIndexHeap(DynArr *heap, int i, int j, comparator compare)
{
	...
	TYPE firstNode = getDynArr(heap, i);
	TYPE secondNode = getDynArr(heap, j);
	if (compare(firstNode, secondNode) == 1)
		return i;
	else
		return j;
}
...
void addHeap(DynArr *heap, TYPE val, comparator  compare)
{
	...
	addDynArr(heap, val);
	New = (sizeDynArr(heap) - 1);				//where we are gonna put the new value, always add at end.
	while (New != 0){
		parent = (New - 1) / 2;
		if (compare(getDynArr(heap, New), getDynArr(heap, parent)) < 0) //if child is greater than parent
		{
			swapDynArr(heap, New, parent);						// switch child and parent
			New = parent;					//run same test against the new parent.(remember New/parent are indexes)
		}else{
			break;
		}
	}
...
void _adjustHeap(DynArr *heap, int max, int pos, comparator compare)
{
	...
	if (rghtIdx < max)
	{
		smallest = _smallerIndexHeap(heap, leftIdx, rghtIdx, compare);			//smallest value of the right or left child of current node
		if (compare(getDynArr(heap, smallest), getDynArr(heap, pos)) < 0){		// if parent is bigger than chosen child
			swapDynArr(heap, smallest, pos);									// swap em
			_adjustHeap(heap, max, pos, compare);								//run it with the new value at pos doe.
		}
	}
	else if (leftIdx < max)
	{
...
void removeMinHeap(DynArr *heap, comparator compare)
{
	...
	int last = heap->size - 1;			//we do -1 because we remove a value right after. We are still 1 over the last existing value.

	putDynArr(heap, 0, getDynArr(heap, last));  //at location 0(the root) put the value that was in the last spot.
	removeAtDynArr(heap, last);
	_adjustHeap(heap, last, 0, compare);		//adjust the heap back into order. remember last is 1 more than the last placed spot.
...
void _buildHeap(DynArr *heap, comparator compare)
{
	...
	_adjustHeap(heap, last, 0, compare);		//put dis ish in order
}
/*
...
void printDynArr(DynArr *v, printer print)
{
  ...
  for(i=0; i < v->size; i++)
    {
	  struct Task* printThis;
	  printThis = (struct Task*)v->data[i];
      printf("DA[%d] == ",i);																//Temporary fixes
     // (*print)(v->data[i]);
      // printf(PRINT_STR, (CAST_STR) v->data[i]);
	  printf("%d, %s\n", printThis->priority, printThis->description);
      printf("\n");
    }
